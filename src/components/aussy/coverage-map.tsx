...
export function CoverageMap({ observerLat, observerLon }: { observerLat: number; observerLon: number }) {
  ...
  return <div className="space-y-4">
    ...
    <Card className="glass-card border-orange-500/20"><CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-sm"><Radio className="h-4 w-4 text-orbit" />Antenas / ERBs<span className="ml-auto"><DataProvenance quality="unavailable" compact /></span></CardTitle></CardHeader><CardContent className="space-y-2 text-xs text-muted-foreground"><p>O Aussy não exibe posições estimadas ou simuladas de ERBs.</p><p>Enquanto não houver integração oficial verificável, essa camada permanece indisponível. Para localização oficial, consulte a base ERB-Web da ANATEL.</p></CardContent></Card>
  </div>
}
